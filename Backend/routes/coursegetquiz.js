...
app.post('/',function(req,res){
   ...
   mongo.connect(url, (err1,con) => {
    ...
    var dbo = con.db("mydb");
    var sql = "SELECT DISTINCT QuizName FROM course_quiz_map1 WHERE CourseId = " +req.body.cid;
    console.log(sql);
    //dbo.collection("course_quiz_map1").find({CourseId : req.body.cid }, {QuizName:1}).toArray(function(err2, result) {
    dbo.collection("course_quiz_map1").distinct("QuizName", {CourseId : req.body.cid }, function(err2, result) {
				if(err2) {
					 throw err2;
