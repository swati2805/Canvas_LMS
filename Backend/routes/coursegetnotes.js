...
mongo.connect(url, function(err1,con) {
    ...
    var dbo = con.db("mydb");
    var sql = "SELECT DISTINCT CourseId, NotesName, FileName FROM course_notes_map WHERE CourseId = " +req.body.cid;
    console.log(sql);
    dbo.collection("course_notes_map").find({CourseId : req.body.cid}, {CourseId : 1, NotesName : 1, FileName : 1}).toArray(function(err2, result){
				if(err2) {
					 throw err2;
