...
app.get('/table6create', (req,res) => {
	...
	if(err1){ throw err1;}
		...
		var sql = "CREATE TABLE course_quiz_map (id INT AUTO_INCREMENT PRIMARY KEY, CourseId  VARCHAR(255), QuizName VARCHAR(255), QuestionNumber VARCHAR(255), RightA VARCHAR(255), A1 VARCHAR(255), A2 VARCHAR(255), A3 VARCHAR(255))";
			con.query(sql, function (err2, result) {
			if (err2) throw err2;
			console.log("course_quiz_map");
			res.send('course_quiz_map table created at poolnodejs database');
		});		
	}	
	});
});
		...
		if(err1){ throw err1;}
			...
			bcrypt.hash(req.body.password, 10, function(err3, encrypted_password) {
			  ...
			  else 
			  {
				...
				var sql = "INSERT INTO profile_table  (first_name,last_name,email,password,city,gender,phone_number,school,hometown,company,languages,country,about_me,ptype) VALUES ( " +mysql.escape(req.body.first_name)+" ,"+mysql.escape(req.body.last_name)+" ,"+mysql.escape(req.body.email)+" ,"+mysql.escape(encrypted_password)+" ,"+mysql.escape(req.body.city)+" ,"+mysql.escape(req.body.gender)+" ,"+mysql.escape(req.body.phone_number)+" ,"+mysql.escape(req.body.school)+" ,"+mysql.escape(req.body.hometown)+" ,"+mysql.escape(req.body.company)+" ,"+mysql.escape(req.body.languages)+" ,"+mysql.escape(req.body.country)+" ,"+mysql.escape(req.body.about_me)+" ,"+mysql.escape(req.body.ptype)+ " )";
				con.query(sql, (err2, result) => {
					if(err2) {
						throw err2;
	...
	console.log("Req Body A3: ",req.body.a3);
			...
			pool.getConnection((err1,con) => {
				if(err1){ throw err1;}
					...
					console.log("inside insert into course_quiz_map table");
		
						var sql = "INSERT INTO course_quiz_map (CourseId, QuizName, QuestionNumber, RightA, A1, A2, A3) VALUES ( "+mysql.escape(req.body.cid)+" ,"+mysql.escape(req.body.quizname)+" ,"+mysql.escape(req.body.qcount)+" ,"+mysql.escape(req.body.right_a)+" ,"+mysql.escape(req.body.a1)+" ,"+mysql.escape(req.body.a2)+" ,"+mysql.escape(req.body.a3)+")";
		                console.log(sql);
						con.query(sql, (err2, result) => {
							if(err2) {
								throw err2;
